...
namespace SatoshisMarketplace.Services.Implementations
{
	public class ProductService : IProductService
	{
		...
		public async Task AddFavoriteProduct(int productId, string username)
		{
			var fp = new Entities.FavoriteProduct()
			{
				ProductId = productId,
				Username = username,
			...
			await _context.FavoriteProducts.AddAsync(fp);
			await _context.SaveChangesAsync();
		}

		public async Task RemoveFavoriteProduct(int productId, string username)
		{
			var fp = await _context.FavoriteProducts.FirstOrDefaultAsync(x => x.ProductId == productId && x.Username == username);

			_context.FavoriteProducts.Remove(fp);
			await _context.SaveChangesAsync();
		}

		public async Task<List<Models.ProductService.ProductModel>> GetFavoriteProductsByUserAsync(string username)
		{
			return await _context.FavoriteProducts
				.Include(fp => fp.Product)
				.Where(fp => fp.Username == username)
				.Select(fp => new Models.ProductService.ProductModel
				{
					Id = fp.Product.Id,
					Name = fp.Product.Name,
					Description = fp.Product.Description,
					IsListed = fp.Product.IsListed,
					FirstPublication = fp.Product.FirstPublication,
					LastUpdate = fp.Product.LastUpdate,
					OwnerUsername = fp.Product.OwnerUsername,
					Price = fp.Product.Price,
					ProductImages = _context.ProductFiles
					.Where(pi => pi.Type == Entities.ProductFileType.ProductImage && pi.ProductId == fp.Product.Id)
					.Select(pi => pi.Id)
					.ToList()
				}).ToListAsync();
		}

		public async Task<bool> IsFavoriteProduct(int productId, string? username)
		{
